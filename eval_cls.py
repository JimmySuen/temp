...
def main(args):
    ...
    if 'swin' in args.arch:
        ...
        args_drop = 0.0
        args_drop_path = 0.1
        args_attn_drop_rate = 0.0
        args_use_mean_pooling = True
        args_init_scale = 0.001
        # args_rel_pos_bias = True
        # args_abs_pos_emb = False
        # args_layer_scale_init_value = 0.1
        ...
        model = create_model(
            ...
            drop_rate=args_drop,
            drop_path_rate=args_drop_path,
            attn_drop_rate=args_attn_drop_rate,
            drop_block_rate=None,
            use_mean_pooling=args_use_mean_pooling,
            init_scale=args_init_scale,
            # use_rel_pos_bias=args_rel_pos_bias,
            # use_abs_pos_emb=args_abs_pos_emb,
            # init_values=args_layer_scale_init_value,
        )

        args_finetune = args.pretrained_weights
        if args_finetune:
            if args_finetune.startswith('https'):
                checkpoint = torch.hub.load_state_dict_from_url(
                    args_finetune, map_location='cpu', check_hash=True)
            else:
                checkpoint = torch.load(args_finetune, map_location='cpu')

            print("Load ckpt from %s" % args_finetune)
            ...
            for key in all_keys:
                ...
                if "relative_position_bias_table" in key:
                    ...
                    dst_patch_shape = (model.patch_embed.img_size[0] // model.patch_embed.patch_size[0],
